...
def species_summary(name,score,count=1,minutes=0,confidence=.8):
    return SpeciesSummary(species_common=name,species_scientific=f"{name} scientific",local_detection_count=count,highest_birdnet_confidence=confidence,latest_detection=datetime.now(timezone.utc)+timedelta(minutes=minutes),best_corroboration_level=None if score is None else "LIKELY",best_corroboration_score=score,display_corroboration_score=display_score(score),nearby_unique_stations=0,nearby_total_detections=0)
...
def test_species_sort_score_then_count_then_latest_with_pending_last():
    values=[species_summary("Pending",None,99,99,1),species_summary("Low",40,20,20),species_summary("High",90),species_summary("Tie lower confidence",74,20,20,.7),species_summary("Tie higher confidence",70,1,-1,.9)]
    ordered=sorted(values,key=summary_sort_key,reverse=True)
    assert [item.species_common for item in ordered]==["High","Tie higher confidence","Tie lower confidence","Low","Pending"]
