...
def serialize(d,detail=False):
    ...
    data=dict(id=d.id,source=d.source,source_detection_id=d.source_detection_id,species_common=d.species_common,species_scientific=d.species_scientific,detected_at=d.detected_at,birdnet_confidence=d.confidence,audio_reference=d.audio_reference,enrichment_state=d.enrichment_state,corroboration=cor)
...
async def latest(limit:int=Query(20,ge=1,le=200),session:Session=Depends(db)):return [serialize(x) for x in query_rows(session,select(LocalDetection).order_by(LocalDetection.detected_at.desc()).limit(limit))]
...
async def today(request:Request,min_confidence:float|None=Query(None,ge=0,le=1),species:str|None=None,corroboration_level:str|None=None,session:Session=Depends(db)):
    ...
    return [serialize(x) for x in query_rows(session,stmt.order_by(LocalDetection.detected_at.desc()).limit(1000))]
