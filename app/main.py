...
def create_app(settings=None,birdnet=None,birdweather=None):
    ...
    bn=birdnet or BirdNetClient(s);bw=birdweather or BirdWeatherClient(s);health=Health();enrichment=EnrichmentService(sessions,bw,s,health);ingestion=IngestionService(sessions,bn,enrichment,s,health)
    @asynccontextmanager
    async def lifespan(app):
        tasks=[asyncio.create_task(ingestion.run(),name="birdnet-ingestion"),asyncio.create_task(enrichment.run(),name="birdweather-enrichment")] if s.worker_enabled else []
        yield
        for task in tasks:task.cancel()
        if tasks:await asyncio.gather(*tasks,return_exceptions=True)
        await bn.close();await bw.close();engine.dispose()
    app=FastAPI(title="Bird Corroborator",version="1.0.0",lifespan=lifespan)
    app.state.settings=s;app.state.sessions=sessions;app.state.health=health;app.state.ingestion=ingestion;app.state.enrichment=enrichment;app.state.metrics=MetricsSampler()
