...
def create_app(settings=None,birdnet=None,birdweather=None,image_provider=None):
    ...
    bn=birdnet or BirdNetClient(s);bw=birdweather or BirdWeatherClient(s);health=Health();enrichment=EnrichmentService(sessions,bw,s,health);ingestion=IngestionService(sessions,bn,enrichment,s,health);image_provider=image_provider or WikimediaCommonsProvider(s);image_service=SpeciesImageService(sessions,image_provider,s)
    @asynccontextmanager
    ...
    app.state.settings=s;app.state.sessions=sessions;app.state.health=health;app.state.ingestion=ingestion;app.state.enrichment=enrichment;app.state.metrics=MetricsSampler();app.state.image_service=image_service
