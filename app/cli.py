...
def main():
    ...
    sub=parser.add_subparsers(dest="command",required=True);p=sub.add_parser("catchup",help="manually import a larger BirdNET history window");p.add_argument("--hours",type=int,required=True);sub.add_parser("images-reset",help="move cached image files aside and queue every species for retrieval")
